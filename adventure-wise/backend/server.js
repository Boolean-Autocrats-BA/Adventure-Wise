...
app.post("/users/trips", async (req, res) => {
  ...
  try {
    const newTrip = await pool.query(
      "INSERT INTO trip (trip_name, user_id) VALUES ($1, $2) RETURNING *",
      [trip_name, user_id]
    );
...
app.get("/users/trips/:id/places", async (req, res) => {
  ...
  try {
    const tripInfo = await pool.query(
      "SELECT * FROM place WHERE trip_id = $1",
      [tripID]
    );
...
app.get("/users/trips/places/:id", async (req, res) => {
  const tripID = req.params.id;
  try {
    const allPlaces = await pool.query(
      "SELECT * FROM trip RIGHT OUTER JOIN place ON trip.trip_id = place.trip_id WHERE trip.trip_id = $1",
      [tripID]
    );
...
app.post("/users/trips/places", async (req, res) => {
  const { place_name, place_address, trip_date, trip_time, notes, trip_id } =
    req.body;
  try {
